...
public:
    ...
    vector<string>value;
    vector<string>assemblyFunctionTempVariableName;
    vector<string>assemblyParameterName;
   //bool isFunction;
    Extra* e;
    string code;
    string assemblyName;
...
ScopeTable(int bucketLength,int tableNo)
{
    tableNumber = tableNo;
    parent = 0;
...
{
    this->bucketLength = bucketLength;
    current = new ScopeTable(bucketLength,1);
    tableNo =1;
    current->setParent(0);
}
...
bool enterScope()
{
    ...
    ScopeTable *child = new ScopeTable(bucketLength,++tableNo);
...
string getCurrentScopeID()
{
   ...
   return to_string(current->tableNumber);

}
int getNextScopeNumber()
{
    return tableNo + 1;
}
